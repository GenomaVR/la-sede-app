...
const solapa = state.reservas.some(r => r.canchaId === canchaId && r.fecha === fecha && !(horaFin <= r.horaInicio || horaInicio >= r.horaFin))
...
// Admin – ABM mínimos (ejemplo usuarios)
const admin = {
crearUsuario: (payload) => setState(prev => ({ ...prev, usuarios: [...prev.usuarios, { id: Date.now(), ...payload }] })),
toggleActivo: (id) => setState(prev => ({
...prev,
usuarios: prev.usuarios.map(u => u.id === id ? ({ ...u, activo: !u.activo }) : u)
})),
// Análogos: canchas, clases, partidos
}


const value = { state, setState, pagarCuota, reservarCancha, inscribirClase, comprarEntradas, admin }
